...
class ListUserAdapter(private val context: Context, private val userList: ArrayList<UserInfo>, private val groupuserList : ArrayList<Group>) : RecyclerView.Adapter<ListUserAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view  = LayoutInflater.from(parent.context).inflate(R.layout.listuser_item,parent,false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val groupUser = cleangroupuserList[position]
        ...
        if(groupUser.groupOwner == userDetails.mail){
            Glide.with(context).load(R.drawable.theboss).into(holder.founderofGroup)
        ...
        if(userDetails.mail?.isNotEmpty() == true) {
            var storageRef = Firebase.storage.getReference(userDetails.mail.toString())
                .downloadUrl.addOnSuccessListener { uri ->
                    ...
                    Handler().post {
                        var hashcontroller = hashSetOf<Int>()
                        var shuffledrandomPfp = randomPfp
                        println(makemeUp)
                        if (makemeUp == 6) {
                            var shuffledrandomPfp = randomPfp.shuffled()
                            makemeUp = 0
                            hashcontroller.clear()
                        } else {
                            if(!(context as Activity).isFinishing) {
                                Glide.with(context).load(shuffledrandomPfp[makemeUp])
                                    .into(holder.pfp)
                                makemeUp++
                                hashcontroller.add(makemeUp)
                            }
                        }
                    }
                }
